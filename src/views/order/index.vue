<template>
  <div class="order-content">
    ...
    <el-card shadow="hover">
      <el-form :inline="true" size="small" :model="searchFrom" class>
        <el-form-item label="买家姓名:" prop>
          <el-input v-model="searchFrom.userName" placeholder="请输入买家姓名" />
        </el-form-item>
        <el-form-item label="买家手机号:">
          <el-input v-model="searchFrom.userPhone" placeholder="请输入买家手机号" />
        </el-form-item>
        <el-form-item label="订单号:">
          <el-input v-model="searchFrom.orderNumber" placeholder="请输入订单号" />
        </el-form-item>
        <el-form-item label="配送方式:">
          <el-select v-model="searchFrom.deliveryMthods" filterable placeholder="请选择">
        ...
        <el-form-item label="订单状态:">
          <el-select v-model="searchFrom.orderStatus" filterable placeholder="请选择">
            <el-option label="全部" value />
            <el-option
        ...
        <el-form-item>
          <el-button class="form-btn__defalut" type="success" @click="searchUser(searchFrom)">查询</el-button>
        </el-form-item>
      </el-form>
    </el-card>
    <el-card style="margin-top:25px;">
      ...
      <el-table :data="tableData" stripe style="width: 100%">
        ...
        <el-table-column prop label="配送方式">
          <template slot-scope="scope">{{ changeDeliveryStatus(scope.row.deliveryMthods) }}</template>
        </el-table-column>
        <el-table-column prop label="订单创建时间" width="200">
          <template slot-scope="scope">{{ formatDate(Number(scope.row.createdTime)) }}</template>
        </el-table-column>
        <el-table-column prop label="订单状态">
          <template slot-scope="scope">
            <span
              :class="{waiting_pay: scope.row.orderStatus === '1', to_be_shipped:scope.row.orderStatus === '2',success: scope.row.orderStatus === '3',close: scope.row.orderStatus === '4'}"
            >{{ changeOrderStatus(scope.row.orderStatus) }}</span>
          </template>
        </el-table-column>
        <el-table-column label="操作" width="300">
          <template slot-scope="scope">
            <el-button type="primary" size="small" @click="toDetail(scope)">查看详情</el-button>
            <el-button
              v-if="scope.row.orderStatus === '2' && scope.row.deliveryMthods === 'delivery'"
              type="warning"
              size="small"
              @click="deliver(scope)"
            >发货</el-button>
            <el-button
              v-if="scope.row.orderStatus === '2' && scope.row.deliveryMthods === 'self'"
              type="warning"
              size="small"
              @click="self(scope)"
            >取货</el-button>
      ...
      />
      <el-dialog title="购买商品详情" :visible.sync="dialogVisible" width="500px">
        <div v-for="(item,index) in goodsData" :key="index" class="el-dialog-div">
...
export default {
  ...
  methods: {
    ...
    self (scope) {
      let _this = this
      this.$confirm('确定订单信息核实无误，客户已来取货?', '警告', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
