<template>
  <div class="goods-content">
    <div class="goods-content-header">
      <h1>商品管理</h1>
      <el-button type="primary" @click="singleCreation">增加商品</el-button>
      <el-button v-if="false" type="primary" @click="batchImport">批量增加商品</el-button>
    </div>
    <el-card shadow="hover">
      <el-form :inline="true" size="small" :model="searchFrom" class>
        <el-form-item label="商品名:" prop>
          <el-input v-model="searchFrom.goodsName" placeholder="请输入商品名" />
        </el-form-item>
        <el-form-item label="价钱:">
          <el-input v-model="searchFrom.goodsPrice" placeholder="请输入价钱" />
        </el-form-item>
        <el-form-item label="商品分类:">
          <el-select v-model="searchFrom.goodsType" filterable placeholder="请选择">
        ...
        <el-form-item label="商品状态:">
          <el-select v-model="searchFrom.goodsStatus" filterable placeholder="请选择">
            <el-option label="全部" value />
            <el-option
    ...
    <el-card style="margin-top:25px;">
      ...
      <el-table :data="tableData" stripe style="width: 100%">
        ...
        <el-table-column prop label="商品分类">
        ...
        <el-table-column prop="goodsStock" label="商品库存" />
        <el-table-column prop label="商品状态">
          <template slot-scope="scope">
            <span
              :class="{
                goods_status_normal: scope.row.goodsStatus === 1,
                goods_status_soldout: scope.row.goodsStatus === 2,
                goods_status_lowershelf: scope.row.goodsStatus === 3
              }"
            >{{ changeGoodsStatus(scope.row.goodsStatus) }}</span>
          </template>
        </el-table-column>
        <el-table-column label="操作" width="300">
          <template slot-scope="scope">
            ...
            <el-button
              v-if="scope.row.goodsStatus === 1 || scope.row.goodsStatus === 2"
              type="danger"
              size="small"
...
export default {
  ...
  methods: {
    ...
    pageGetGoodsFun (e) {
      ...
      pageGetGoods("goods/pageGetGoods", data)
        .then(res => {
          closeLoading();
          if (res.data.data) {
    ...
    queryGoodsFun (searchFrom) {
      let _this = this;
      startLoading();
      queryGoods("goods/queryGoods", searchFrom)
        .then(res => {
          closeLoading();
          if (res.data.data) {
            _this.tableData = changeQuerystring(res.data.data);
    ...
    edit (scope) {
      this.$router.push(
        `/createGoods?type=edit&id=${this.Config.ENCODE(scope.row.id)}`
      );
    },
    // 取消发布
    handleClose () {
